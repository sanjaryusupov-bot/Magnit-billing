...
from data_parsing import parse_workbook_bytes, build_sla
...
def _load_from_gsheet(sheet_id: str):
    url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=xlsx"
    resp = requests.get(url, timeout=60)
    resp.raise_for_status()
    return parse_workbook_bytes(resp.content)


# ---------------------------------------------------------------------------
# Sidebar — источник данных (только Google Sheet)
# ---------------------------------------------------------------------------
...
sheet_id = st.sidebar.text_input(
    "ID Google-таблицы",
    value=default_id,
    help="ID — часть ссылки между /d/ и /edit. Таблица должна быть доступна "
    "'Всем, у кого есть ссылка — Читатель' (Настройки доступа в Google Sheets), "
    "иначе выгрузка не сработает.",
)
...
if st.sidebar.button("🔄 Загрузить / обновить данные", use_container_width=True):
    _load_from_gsheet.clear()

if sheet_id:
    try:
        ship_df, orders_df = _load_from_gsheet(sheet_id)
    except requests.exceptions.HTTPError:
        st.sidebar.error(
            "Не удалось скачать таблицу (доступ закрыт?). Откройте доступ "
            "'Всем, у кого есть ссылка — Читатель' и нажмите обновить."
        )
    except KeyError as e:
        st.sidebar.error(f"В таблице не найден лист {e}. Проверьте названия вкладок.")
