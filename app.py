...
with tab4:
    ...
    st.markdown(
        """
Один лист **Реестр** — построчно по каждому заказу (Дата отгрузки, Заказ,
Рейс, Кол-во штук, ID магазина, Магазин, Адрес, Координаты, Регион,
Регион тариф, Авто, 1 точка, 2 точка и далее, Грузчик экспедитор,
Итого сумма). Оформлен как настоящая таблица Excel — при необходимости
на его основе легко за пару кликов строится сводная таблица.
        """
    )

    billing_bytes = build_billing_workbook(f)
    st.download_button(
