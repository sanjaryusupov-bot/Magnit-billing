...
with tab4:
    ...
    st.markdown(
        """
Файл будет содержать несколько листов:

- **Отгрузки по дням** — итог за период (как в первой вкладке).
- **Детали по магазинам** — Дата, Магазин, Кол-во шт, Сумма, Сумма за шт — за весь выбранный период.
- **Сводная** — Город / Транспорт / Рейс / сумма / кол-во.
- **SLA план-факт** — если доступен лист «Статус по заказам».
- **Реестр (аудит)** — построчные данные по каждому заказу в каждом рейсе:
  Дата, Рейс, Заказ, Магазин, Кол-во шт, исходный тариф, сумма и кол-во штук
  по всему рейсу, тариф за штуку и итоговая распределённая сумма. По этому
  листу можно пересчитать вручную любую цифру из отчёта.
- **Как считается** — короткое текстовое пояснение формулы.
        """
    )
