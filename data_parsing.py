...
def add_route_economics(ship_df: pd.DataFrame, region_tariff_map: dict = None) -> pd.DataFrame:
    ...
    ДВА варианта идентификатора маршрута:
        ...
        - Маршрут_ID — "умный" вариант с разделением ПО РЕГИОНУ, но СТРОГО в
          ...
          внутри одного "Рейс N" встретились заказы из разных тарифных зон
          (Регион_тариф) — каждая зона становится отдельным маршрутом
          (Маршрут_ID = номер первого заказа этой зоны в блоке); если в
          блоке всего одна тарифная зона, все заказы блока получают один
          и тот же Маршрут_ID, совпадающий с Маршрут_ID_первый_заказ (т.е.
          колонки "Рейс" и "Рейс 2" в биллинг-отчёте будут одинаковыми).
          Используется для расчёта Тариф_за_шт / Сумма_распределенная и
          для доплаты грузчика (это финансово корректная группировка).
    ...
    Доплата "грузчик-экспедитор" (LOADER_FEE_AMOUNT = 440 000):
        ...
        - Применяется ТОЛЬКО к маршрутам, у которых Регион_тариф входит в
          ...
          "Фергана"). Для всех остальных регионов, включая Ташкент и его
          спутники (Ангрен, Сырдарья, Ташобласть - Нурафшон,
          Ташобласть - Чирчик, Янгиюль), "Грузчик экспедитор" и "Итого
          сумма" НЕ трогаются вообще — остаются ровно такими, какими
          пришли из исходной таблицы.
        - Для маршрутов из LONG_HAUL_LOADER_FEE_REGIONS поле "Грузчик
          ...
          маршруту >= 3000, ровно одной (якорной) строке маршрута
          проставляется 440 000, у всех остальных строк маршрута — 0.
...
[19.08.2026 14:31] Санжар: 
    ...
    # Маршрут_ID: группируем СТРОГО по Регион_тариф, но В РАМКАХ ОДНОГО
    # исходного блока "Рейс N" (Дата, Рейс) — если внутри одного "Рейс N"
    # встретились заказы из разных тарифных зон, каждая зона становится
    # отдельным маршрутом (Маршрут_ID = номер первого заказа этой зоны
    # внутри блока); если в блоке всего одна тарифная зона — все заказы
    # блока получают один и тот же Маршрут_ID, совпадающий с
    # Маршрут_ID_первый_заказ (т.е. "Рейс" и "Рейс 2" одинаковы).
    ...
    # LONG_HAUL_LOADER_FEE_REGIONS. Для всех остальных регионов (в т.ч.
    # Ташкент и его спутники) значения остаются как в исходной таблице.
