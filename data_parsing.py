...
# сворачиваются в тарифную зону "Ташкент" (сам Ташкент плюс спутники:
# Ангрен, Сырдарья, Ташобласть - Нурафшон, Ташобласть - Чирчик, Янгиюль).
# Для этой зоны логика автодоплаты "грузчик-экспедитор" НЕ применяется —
# поля "Грузчик экспедитор"/"Итого сумма" берутся из исходной
# Google-таблицы как есть, без пересчёта (см. add_route_economics).
TASHKENT_ZONE_TARIFF = "Ташкент"

# Порог кол-ва штук на маршрут, при достижении/превышении которого на
# маршрут положена доплата "грузчик-экспедитор" — но только для регионов
# ВНЕ ташкентской зоны (см. TASHKENT_ZONE_TARIFF выше).
...
def add_route_economics(ship_df: pd.DataFrame, region_tariff_map: dict = None) -> pd.DataFrame:
    ...
    Доплата "грузчик-экспедитор" (LOADER_FEE_AMOUNT = 440 000):
        - Считается по маршруту (Дата, Маршрут_ID) — суммарное кол-во штук
          по маршруту.
        - Применяется ТОЛЬКО если Регион_тариф маршрута НЕ "Ташкент" (см.
          TASHKENT_ZONE_TARIFF) — т.е. для ташкентской зоны (Ташкент,
          Ангрен, Сырдарья, Ташобласть - Нурафшон, Ташобласть - Чирчик,
          Янгиюль — все они по region_tariff_map сворачиваются в "Ташкент")
          "Грузчик экспедитор" и "Итого сумма" НЕ трогаются вообще —
          остаются ровно такими, какими пришли из исходной таблицы.
        - Для остальных регионов поле "Грузчик экспедитор" пересчитывается
          с нуля (любое ошибочно проставленное в исходнике значение
          убирается): если суммарное кол-во штук по маршруту >= 3000,
          ровно одной (якорной) строке маршрута проставляется 440 000, у
          всех остальных строк маршрута — 0. "Итого сумма" для этих строк
    ...
    # исходной таблице. Для остальных регионов пересчитываем с нуля (убирая
    # любое ошибочно проставленное в исходнике значение).
