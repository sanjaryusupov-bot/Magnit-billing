...
def add_route_economics(ship_df: pd.DataFrame, region_tariff_map: dict = None) -> pd.DataFrame:
    ...
    ДВА варианта идентификатора маршрута:
        - Маршрут_ID_первый_заказ — простой вариант: номер ПЕРВОГО заказа,
          ...
          "Рейс" биллинг-отчёта.
        - Маршрут_ID — "умный" вариант с разделением: новый маршрут
          начинается, когда в строке ненулевая "1 точка" ИЛИ когда
          Регион_тариф этой строки отличается от региона текущего
          маршрута (например, чередование Наманган/Фергана в одном
          "Рейс N" — это разные маршруты, а не один с несколькими точками).
          Идёт в колонку "Рейс 2" биллинг-отчёта и используется для расчёта
          Тариф_за_шт/Сумма_распределенная (это финансово корректная
          группировка).
    ...
    # Маршрут_ID: новый маршрут начинается, когда в строке ненулевая
    # "1 точка" (явный маркер нового базового тарифа), ИЛИ когда регион
    # (тарифная зона) этой строки отличается от региона текущего маршрута —
    # разные регионы в пределах одного "Рейс N" не могут быть одним
    # физическим маршрутом, даже если "1 точка" не проставлена.
    def _assign_route_ids(g: pd.DataFrame) -> pd.Series:
        orders = g["Заказ"].tolist()
        tochka1 = g["Точка_1"].tolist()
        regions = g["Регион_тариф"].tolist()

        result = []
        current_id = None
        current_region = None
        for order, t1, region in zip(orders, tochka1, regions):
            is_new = (
                current_id is None
                or (t1 is not None and t1 > 0)
                or (region is not None and current_region is not None and region != current_region)
            )
            if is_new:
                current_id = order
