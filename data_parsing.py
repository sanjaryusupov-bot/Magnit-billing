...
# Регионы (по Регион_тариф), для которых при кол-ве штук >= 3000 на маршрут
# автоматически доплачивается "грузчик-экспедитор" (см. add_route_economics).
# "Коканд" в этот список отдельно не входит, т.к. по DEFAULT_REGION_TARIFF_MAP
# он уже сворачивается в тарифную зону "Фергана" (которая в списке есть).
# Для ВСЕХ остальных регионов, включая Ташкент и его спутники (Ангрен,
# Сырдарья, Ташобласть - Нурафшон, Ташобласть - Чирчик, Янгиюль — они по
# карте тарифов тоже сворачиваются в "Ташкент"), доплата вообще не
# трогается — "Грузчик экспедитор"/"Итого сумма" остаются как в исходной
# Google-таблице, без пересчёта.
LONG_HAUL_LOADER_FEE_REGIONS = {"Самарканд", "Фергана", "Наманган", "Андижан", "Навои", "Бухара"}

# Порог кол-ва штук на маршрут, при достижении/превышении которого на
# маршрут (из LONG_HAUL_LOADER_FEE_REGIONS) положена доплата
# "грузчик-экспедитор".
...
def add_route_economics(ship_df: pd.DataFrame, region_tariff_map: dict = None) -> pd.DataFrame:
    ...
    ДВА варианта идентификатора маршрута:
        - Маршрут_ID_первый_заказ — простой вариант: номер ПЕРВОГО заказа,
          ...
          разделения. Идёт в колонку "Рейс" биллинг-отчёта. Отражает
          исходную разбивку по "Рейс N" из самой Google-таблицы как есть.
        - Маршрут_ID — "умный" вариант с разделением ПО РЕГИОНУ, но СТРОГО в
          рамках одного блока "Рейс N" исходной таблицы (Дата, Рейс), И
          ТОЛЬКО если весь блок относится к ОДНОЙ тарифной зоне
          (Регион_тариф) — тогда, если внутри блока встретились заказы из
          разных исходных регионов (например, Ташкент + Ангрен — обе точки
          относятся к зоне "Ташкент", но физически разные), каждый регион
          становится отдельным маршрутом (Маршрут_ID = номер первого
          заказа этого региона в блоке); если в блоке всего один регион,
          все заказы блока получают один и тот же Маршрут_ID, совпадающий
          с Маршрут_ID_первый_заказ (т.е. "Рейс" и "Рейс 2" одинаковы).

          ИСКЛЮЧЕНИЕ — "сквозные" рейсы: если один и тот же блок "Рейс N"
          охватывает НЕСКОЛЬКО РАЗНЫХ тарифных зон (Регион_тариф) — то есть
          машина по факту едет транзитом через несколько дальних городов
          (например, Самарканд -> Навои -> Бухара в одном рейсе) — такой
          рейс НЕ разбивается на Рейс 2 вообще: Маршрут_ID для всех его
          строк = Маршрут_ID_первый_заказ (т.е. "Рейс" и "Рейс 2" опять
          совпадают, показываем как есть). См. отдельный блок про
          распределение суммы и доплату грузчика для таких рейсов ниже.
    ...
    Доплата "грузчик-экспедитор" (LOADER_FEE_AMOUNT = 440 000):
        - Считается по маршруту (Дата, Маршрут_ID) — суммарное кол-во штук
          по маршруту.
        - Применяется ТОЛЬКО к маршрутам, у которых Регион_тариф входит в
          LONG_HAUL_LOADER_FEE_REGIONS = {Самарканд, Фергана, Наманган,
          Андижан, Навои, Бухара} (Коканд сюда же, т.к. сворачивается в
          "Фергана"), И которые НЕ являются "сквозными" (см. выше). Для
          всех остальных маршрутов — включая Ташкент и его спутники
          (Ангрен, Сырдарья, Ташобласть - Нурафшон, Ташобласть - Чирчик,
          Янгиюль), а также ЛЮБЫЕ "сквозные" рейсы (даже если среди их
          регионов есть дальнобойные) — "Грузчик экспедитор" и "Итого
          сумма" НЕ трогаются вообще, остаются ровно такими, какими пришли
          из исходной Google-таблицы.
        - Для обычных (не сквозных) маршрутов из LONG_HAUL_LOADER_FEE_REGIONS
          поле "Грузчик экспедитор" пересчитывается с нуля (любое ошибочно
          ...
          0. "Итого сумма" для этих строк пересчитывается как "1 точка" +
          "2 точка и далее" + "Грузчик экспедитор".

    Распределение суммы по заказам:
        - Если в исходном блоке (Дата, Рейс) ВСЕГО ОДИН регион — как раньше,
          пропорционально количеству штук в заказе (Тариф_за_шт =
          Сумма_маршрута / Кол_во_шт_маршрута; Сумма_распределенная =
          Кол_во_шт * Тариф_за_шт).
        - Если в блоке НЕСКОЛЬКО регионов (независимо от того, разбился он
          на Рейс 2 или нет — см. "сквозные" выше) — сумма ВСЕГО БЛОКА
          делится ПОРОВНУ между уникальными заказами этого блока (а не по
          штукам и не по под-маршруту), т.к. в исходнике вся сумма рейса
          часто висит одной строкой на первом заказе, а остальные регионы
          получают 0 — из-за этого расчёт "по под-маршруту" давал бы
          неверные (нулевые) суммы для остальных регионов/заказов. Если
          один и тот же заказ занимает несколько строк, его доля делится
          поровну между его собственными строками.
    ...
    # "Сквозной" рейс — блок (Дата, Рейс), который охватывает несколько
    # РАЗНЫХ тарифных зон одновременно (машина едет транзитом через
    # несколько дальних городов). Такие рейсы не разбиваем на Рейс 2.
    n_zones_in_block = df.groupby(
        ["Дата", "Рейс"], dropna=False
    )["Регион_тариф"].transform("nunique")
    df["Маршрут_сквозной"] = n_zones_in_block > 1

    # Маршрут_ID: группируем СТРОГО по исходному полю "Регион" (не по
    # свёрнутой тарифной зоне Регион_тариф!), но В РАМКАХ ОДНОГО исходного
    # блока "Рейс N" (Дата, Рейс) — если внутри одного "Рейс N" встретились
    # заказы из разных регионов ОДНОЙ тарифной зоны (например,
    # Ангрен/Сырдарья/Ташобласть - Нурафшон/Ташобласть - Чирчик — они хоть и
    # относятся к одной тарифной зоне "Ташкент", физически это разные
    # точки), каждый регион становится отдельным маршрутом (Маршрут_ID =
    # номер первого заказа этого региона внутри блока); если в блоке всего
    # один регион — все заказы блока получают один и тот же Маршрут_ID,
    # совпадающий с Маршрут_ID_первый_заказ. Для "сквозных" блоков (см.
    # выше) разбиение вообще не делаем — Маршрут_ID = Маршрут_ID_первый_заказ
    # для всех строк блока.
    def _assign_route_ids(g: pd.DataFrame) -> pd.Series:
        orders = g["Заказ"].tolist()
        regions = g["Регион"].tolist()
        region_to_anchor = {}
        result = []
    ...
    marshrut_id = df["Маршрут_ID_первый_заказ"].copy()
    non_through_idx = df.index[~df["Маршрут_сквозной"]]
    for _, idx in df.loc[non_through_idx].groupby(["Дата", "Рейс"], dropna=False, sort=False).groups.items():
    ...
    is_long_haul = df["Регион_тариф"].isin(LONG_HAUL_LOADER_FEE_REGIONS)

    # Пересчитываем "Грузчик экспедитор"/"Итого сумма" с нуля (убирая любое
    # ошибочно проставленное в исходнике значение) ТОЛЬКО для маршрутов из
    # LONG_HAUL_LOADER_FEE_REGIONS, которые при этом НЕ сквозные. Для всех
    # остальных случаев (Ташкент и его спутники, а также любые сквозные
    # рейсы) значения остаются как в исходной таблице.
    recalc_mask = is_long_haul & ~df["Маршрут_сквозной"]
    ...
    # Кол-во строк на заказ и кол-во уникальных заказов — считаем на уровне
    # ВСЕГО ИСХОДНОГО БЛОКА (Дата, Рейс), а не под-маршрута: если в рейсе
    # несколько регионов, вся сумма рейса в исходнике часто висит одной
    # строкой на первом заказе (остальные регионы/точки получают 0), из-за
    # чего расчёт "по под-маршруту" давал бы неверные (нулевые) суммы для
    # остальных регионов. Поэтому распределяем поровну по заказам всего
    # блока — независимо от того, был ли блок разбит на несколько
    # Маршрут_ID или нет.
    block_cols = ["Дата", "Рейс"]
    df["_rows_per_order"] = df.groupby(block_cols + ["Заказ"], dropna=False)["Заказ"].transform("size")
    df["_orders_per_block"] = df.groupby(block_cols, dropna=False)["Заказ"].transform("nunique")
    df["_sum_per_block"] = df.groupby(block_cols, dropna=False)["Итого_сумма"].transform("sum")
    # Кол-во РАЗНЫХ регионов в исходном блоке (Дата, Рейс) — если больше
    # одного, распределяем сумму поровну по заказам (см. выше), независимо
    # от того, одна это тарифная зона (местный сплит: Ташкент/Ангрен/
    # Сырдарья/...) или несколько (сквозной: Самарканд/Навои/Бухара/...).
    n_regions_in_block = df.groupby(block_cols, dropna=False)["Регион"].transform("nunique")
    df["Маршрут_многорегиональный"] = n_regions_in_block > 1
    ...
    # Однорегиональные маршруты — как раньше, пропорционально штукам.
    # Многорегиональные (в т.ч. сквозные) — поровну между уникальными
    # заказами ВСЕГО исходного блока (Дата, Рейс) (доля каждого заказа
    # делится поровну между его собственными строками, если их несколько).
    df["Сумма_распределенная"] = df["Кол_во_шт"] * df["Тариф_за_шт"]
    multi_mask = df["Маршрут_многорегиональный"]
    df.loc[multi_mask, "Сумма_распределенная"] = (
        df.loc[multi_mask, "_sum_per_block"]
        / df.loc[multi_mask, "_orders_per_block"]
        / df.loc[multi_mask, "_rows_per_order"]
    )

    df = df.drop(columns=["_rows_per_order", "_orders_per_block", "_sum_per_block"])
