...
def add_route_economics(ship_df: pd.DataFrame, region_tariff_map: dict = None) -> pd.DataFrame:
    ...
    ДВА варианта идентификатора маршрута:
        ...
        - Маршрут_ID — "умный" вариант с разделением ПО РЕГИОНУ, СТРОГО в
          рамках одного блока "Рейс N" исходной таблицы (Дата, Рейс): если
          внутри блока встретились заказы из разных исходных регионов
          (например, Самарканд + Бухара + Навои, или Ташкент + Ангрен),
          каждый регион становится ОТДЕЛЬНЫМ маршрутом (Маршрут_ID = номер
          первого заказа этого региона внутри блока, в порядке появления
          строк). Если в блоке всего один регион, все заказы блока получают
          один и тот же Маршрут_ID, совпадающий с Маршрут_ID_первый_заказ
          (т.е. "Рейс" и "Рейс 2" одинаковы).

          Это разделение применяется ВСЕГДА, когда в блоке несколько
          регионов — независимо от того, относятся ли они к одной тарифной
          зоне (например, Ташкент/Ангрен/Чирчик) или к разным дальним
          зонам одновременно (например, Самарканд/Навои/Бухара в одном
          "Рейсе" исходника, когда машина по факту едет транзитом через
          несколько городов). Отдельного "сквозного" исключения больше нет:
          такие смешанные рейсы тоже разбиваются на Рейс 2 по регионам, и
          доплата "грузчик-экспедитор" считается уже по каждому
          получившемуся под-маршруту отдельно (см. ниже).
    ...
    Доплата "грузчик-экспедитор" (LOADER_FEE_AMOUNT = 440 000):
        - Считается по (уже разделённому) маршруту (Дата, Маршрут_ID) —
          суммарное кол-во штук по этому под-маршруту.
        - Применяется ТОЛЬКО к под-маршрутам, у которых Регион_тариф входит
          в LONG_HAUL_LOADER_FEE_REGIONS = {Самарканд, Фергана, Наманган,
          Андижан, Навои, Бухара} (Коканд сюда же, т.к. сворачивается в
          "Фергана"). Для всех остальных под-маршрутов — включая Ташкент и
          его спутники (Ангрен, Сырдарья, Ташобласть - Нурафшон,
          Ташобласть - Чирчик, Янгиюль) — "Грузчик экспедитор" и "Итого
          сумма" НЕ трогаются вообще, остаются ровно такими, какими пришли
          из исходной Google-таблицы.
        - Для под-маршрутов из LONG_HAUL_LOADER_FEE_REGIONS поле "Грузчик
          экспедитор" пересчитывается с нуля (любое ошибочно проставленное
          в исходнике значение убирается): если суммарное кол-во штук по
          под-маршруту >= 3000, ровно одной (якорной) строке под-маршрута
          проставляется 440 000, у всех остальных строк под-маршрута — 0.
          "Итого сумма" для этих строк пересчитывается как "1 точка" +
    ...
    Распределение суммы по заказам:
        - Если в исходном блоке (Дата, Рейс) ВСЕГО ОДИН регион — как раньше,
          ...
          Кол_во_шт * Тариф_за_шт), считается уже по под-маршруту
          (Дата, Маршрут_ID).
        - Если в блоке НЕСКОЛЬКО регионов — сумма ВСЕГО БЛОКА делится
          ПОРОВНУ между уникальными заказами этого блока (а не по штукам и
          не по под-маршруту), т.к. в исходнике вся сумма рейса часто
          висит одной строкой на первом заказе, а остальные регионы
    ...
    именно это и есть настоящий физический под-маршрут (в отличие от
    "Рейс N", который может объединять несколько разных маршрутов с разными
    тарифами, либо наоборот — один и тот же маршрут/регион может быть
    искусственно разбит исходной таблицей на несколько "Рейс N").
    """
    if ship_df.empty:
        return ship_df
    ...
    # и НЕ меняется при разделении по регионам ниже.
    ...
    # заказы из разных регионов (будь то разные точки одной тарифной зоны,
    # например Ангрен/Сырдарья/Ташобласть - Нурафшон/Ташобласть - Чирчик,
    # либо совсем разные дальние зоны, например Самарканд/Навои/Бухара в
    # одном "сквозном" рейсе), каждый регион становится отдельным
    # под-маршрутом (Маршрут_ID = номер первого заказа этого региона внутри
    # блока, в порядке появления строк). Если в блоке всего один регион —
    # все заказы блока получают один и тот же Маршрут_ID, совпадающий с
    # Маршрут_ID_первый_заказ. Разделение применяется всегда, без
    # исключений для "сквозных" многозонных рейсов.
    ...
    # ошибочно проставленное в исходнике значение) для КАЖДОГО под-маршрута
    # из LONG_HAUL_LOADER_FEE_REGIONS — теперь без исключения для "сквозных"
    # рейсов: раз рейс уже разбит на под-маршруты по регионам, доплата
    # считается по каждому под-маршруту отдельно. Для под-маршрутов вне
    # LONG_HAUL_LOADER_FEE_REGIONS (Ташкент и его спутники) значения
    # остаются как в исходной таблице.
    recalc_mask = is_long_haul
    ...
    # одного, распределяем сумму поровну по заказам (см. выше).
    ...
    # Многорегиональные — поровну между уникальными заказами ВСЕГО
    # исходного блока (Дата, Рейс) (доля каждого заказа делится поровну
    # между его собственными строками, если их несколько).
