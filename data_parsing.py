...
def add_route_economics(ship_df: pd.DataFrame, region_tariff_map: dict = None) -> pd.DataFrame:
    ...
    Распределение суммы по заказам:
        - Для ОБЫЧНЫХ (не сквозных) маршрутов — как раньше, пропорционально
          количеству штук в заказе (Тариф_за_шт = Сумма_маршрута /
          Кол_во_шт_маршрута; Сумма_распределенная = Кол_во_шт * Тариф_за_шт).
        - Для "СКВОЗНЫХ" маршрутов — сумма маршрута делится ПОРОВНУ между
          уникальными заказами маршрута (а не по штукам), т.к. в разных
          дальних городах "штука" физически не эквивалентна по стоимости
          доставки. Если один и тот же заказ занимает несколько строк —
          его доля делится поровну между его собственными строками.
    ...
    # Кол-во строк на заказ и кол-во уникальных заказов в маршруте — нужно
    # для равного распределения суммы у сквозных рейсов (см. ниже).
    df["_rows_per_order"] = df.groupby(grp_cols + ["Заказ"], dropna=False)["Заказ"].transform("size")
    df["_orders_per_route"] = df.groupby(grp_cols, dropna=False)["Заказ"].transform("nunique")
    ...
    # Обычные маршруты — пропорционально штукам. Сквозные — поровну между
    # уникальными заказами маршрута (доля каждого заказа делится поровну
    # между его собственными строками, если их несколько).
    df["Сумма_распределенная"] = df["Кол_во_шт"] * df["Тариф_за_шт"]
    through_mask = df["Маршрут_сквозной"]
    df.loc[through_mask, "Сумма_распределенная"] = (
        df.loc[through_mask, "Сумма_маршрута"]
        / df.loc[through_mask, "_orders_per_route"]
        / df.loc[through_mask, "_rows_per_order"]
    )

    df = df.drop(columns=["_rows_per_order", "_orders_per_route"])
