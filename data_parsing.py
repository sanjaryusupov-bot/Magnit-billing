...
def add_route_economics(ship_df: pd.DataFrame, region_tariff_map: dict = None) -> pd.DataFrame:
    ...
    Дополнительно группировка учитывает "Регион": в исходнике иногда один и
    тот же номер рейса по ошибке проставлен заказам из РАЗНЫХ городов (один
    развоз физически не может быть одновременно в Коканде и Намангане) —
    в этом случае рейс разбивается по региону, и заказ(ы) из "чужого"
    города становятся отдельным маршрутом со своим Маршрут_ID (якорь —
    первый заказ этого города в исходном порядке строк), а не размазывают
    тариф на заказы из другого города.
    """
    if ship_df.empty:
        return ship_df
