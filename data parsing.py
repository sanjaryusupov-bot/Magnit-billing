...
def add_route_economics(ship_df: pd.DataFrame, region_tariff_map: dict = None) -> pd.DataFrame:
    ...
    ДВА варианта идентификатора маршрута:
        ...
        - Маршрут_ID — "умный" вариант с разделением ПО РЕГИОНУ, но уже в
          рамках ВСЕГО ДНЯ (Дата), а не одного блока "Рейс N": все строки
          за день с одинаковым Регион_тариф считаются ОДНИМ маршрутом
          (Маршрут_ID = номер первого заказа этого региона за день),
          независимо от того, в каком блоке "Рейс N" исходной таблицы они
          физически находились. Идёт в колонку "Рейс 2" биллинг-отчёта и
    ...
    # Маршрут_ID: группируем СТРОГО по Регион_тариф в рамках ВСЕГО ДНЯ (а не
    # в рамках одного блока "Рейс N") — все строки одного региона за день
    # получают номер первого заказа этого региона за день, даже если
    # исходно они были в разных блоках "Рейс N" таблицы.
